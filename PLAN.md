...
- `bend f.bend` checks the file, then runs `main`. `-o out` builds. `--checkup`
  ...
  its 125 laws as TODOs. Use `bend PROOF.bend`, never `--checkup`, for the gate.
...
125 laws, every one discharged. Nine are reflexivity proofs (`{==}`, both sides
...
- [ ] **S1** Chunk-window migration · **in progress** · deps: `scale` P3 · serves: scale
  ...
  **Next slice: W3 (live dirty-set rewire, `G-scale-2` item 1) — the full
  W1–W6 handoff is `SCALE.md` §8.** Each step gate-green; gap candidates
