...
- [ ] **S1** Chunk-window migration · **in progress** · deps: `scale` P3 · serves: scale
  ...
  **`G-scale-2` item (3) core landed (A.107):** `src/maskword.bend` proves the
  ...
  Remaining: **step 5** live 64-row rewire + the `Mask ↔ Word(64)` index bridge
  (`Word.to_nat`/`Word.inc`), **step 6** torus-law retirement, `G-scale-8`'s live
  load/evict wiring. Each step gate-green; gap candidates
