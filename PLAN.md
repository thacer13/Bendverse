...
- [ ] **S1** Chunk-window migration · **in progress** · deps: `scale` P3 · serves: scale
  ...
  **W5 landed (A.112, parallel track `w5`):** the pure live motion driver
  ...
  `gsc1`):** law `gsc1_axis_no_fold` (guard `True` ⇒ no fold). **Next slices:**
  wire the driver into `runners/serve.bend` (the renderer's live link), then W3
  (live dirty-set rewire, `G-scale-2` item 1) and W6 (torus retirement) — the
  full W1–W6 handoff is `SCALE.md` §8.** Each step gate-green; gap candidates
