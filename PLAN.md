...
- `bend f.bend` checks the file, then runs `main`. `-o out` builds. `--checkup`
  ...
  its 121 laws as TODOs. Use `bend PROOF.bend`, never `--checkup`, for the gate.
...
121 laws, every one discharged. Nine are reflexivity proofs (`{==}`, both sides
...
- [ ] **S1** Chunk-window migration · **in progress** · deps: `scale` P3 · serves: scale
  ...
  **`G-scale-8` store load/evict policy landed (A.108):** `Store.window_store`
  ...
  evict leaving; T120/T121). Remaining: **step 5** live 64-row rewire + the
  `Mask ↔ Word(64)` index bridge (`Word.to_nat`/`Word.inc`), **step 6** torus-law
  retirement, `G-scale-8`'s wiring of the policy *into the live `Sim` loop*. Each step gate-green; gap candidates
