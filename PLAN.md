...
- `bend f.bend` checks the file, then runs `main`. `-o out` builds. `--checkup`
  ...
  its 90 laws as TODOs. Use `bend PROOF.bend`, never `--checkup`, for the gate.
...
90 laws, every one discharged. Six are reflexivity proofs (`{==}`, both sides
...
- [ ] **V3c** Schedule invariance: a region-split fold equals the sequential · optional · deps: — · serves: R10
  ...
  **V3c-2 structural half done (A.87):** `v3c_ne_idx_sep`/`_same_l`/`_same_r`
  (the `ne_idx` node recursion) plus the unconditional `add ∘ sub` roundtrip
  (`v3c_add_sub`); T34/T35. Remaining, **only if `M7d` is pursued**: the three
  `U32` order/range bricks — `is_lt` base reflection (`i < 1 ⇒ i == 0`), range
  preservation under the right shift, and `shr(v3c_two(k+1)) == v3c_two(k)` — and
  wiring the engine's `Rules.Write` list into the fold. The proven
  parallel-safety contract remains V3a+V3b. `G3` gates only `M7d`, **not `C3`**.
