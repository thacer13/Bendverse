...
- `bend f.bend` checks the file, then runs `main`. `-o out` builds. `--checkup`
  ...
  its 84 laws as TODOs. Use `bend PROOF.bend`, never `--checkup`, for the gate.
...
84 laws, every one discharged. Six are reflexivity proofs (`{==}`, both sides
...
- [ ] **V3c** Schedule invariance: a region-split fold equals the sequential · optional · deps: — · serves: R10
  ...
  *composition*: applying the merged write set is order-independent. **V3c-1
  ...
  Remaining, **only if `M7d` is pursued**: V3c-2 *completeness* — distinct
  in-range indices on a full tree are separated (needs `U32` order/subtraction
  arithmetic) — and wiring the engine's `Rules.Write` list into the fold. The
  proven parallel-safety contract remains V3a+V3b. `G3` gates only `M7d`, **not
  `C3`**.
