...
- `bend f.bend` checks the file, then runs `main`. `-o out` builds. `--checkup`
  ...
  its 126 laws as TODOs. Use `bend PROOF.bend`, never `--checkup`, for the gate.
...
126 laws, every one discharged. Nine are reflexivity proofs (`{==}`, both sides
