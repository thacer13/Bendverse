...
- `bend f.bend` checks the file, then runs `main`. `-o out` builds. `--checkup`
  ...
  its 139 laws as TODOs. Use `bend PROOF.bend`, never `--checkup`, for the gate.
...
139 laws, every one discharged. Nine are reflexivity proofs (`{==}`, both sides
definitionally equal):

`sanity`, `grid_volume`, `cell_full_mask`, `cell_reserved_bits`,
`rock_crumbles_lighter`, `sand_sinks_in_water`, `dwin_width_n6`,
`dwin_mark_if_true`, `dwin_mark_if_false`.
...
| `R11` | Worldgen is a pure terrain seeding function (the ground is **world**-fixed; only the boundary is a grid/window property) | — (purity by construction) | T6 T7 T8 T16 T18 | worldgen chunk store |
...
| `R11` | Worldgen is a pure terrain seeding function | conforming | purity by construction: `Worldgen.gen_terrain` samples the coordinate alone, so the terrain is shell-free and window-independent. **The ground is not the boundary.** The old "shell" conflated closure (contract 2 — legitimately a box/window property) with the support base case (rules 5–7 — a **world** property); a ground at the window edge would follow the camera. The closed reference box keeps its historical shell at `{0,63}` bit-for-bit (`win_gen_canonical` — **retired A.123**, kept as a record), so the fixed-box worlds and laws stand. The live window's world-fixed ground is `G-scale-9`; the window-edge-shell variant (`src/winshell.bend` `gen_at`) is **retired** — the wrong direction for the ground (camera-relative gravity) |
...
**Retirement (A.118, S1/W3).** The live `Dirty.Mask` rewire (W3) replaced the two-`U32` record `Mask{lo, hi}` with `Mask{w: Word(64n)}`, so `Dirty.is_dirty_y`/`or_y`/`mark_wake` now delegate to `Dirtyn` on the held word. The law `mask_word_get` became **definitional** (`{==}`): still true, no longer content. The `wordnat` lo/hi composition lemmas stated about `Dirty.Mask{lo, hi}`/`Dirty.lo_of`/`hi_of` (`wnat_dirty_true`/`_false`, `wnat_mask_get_lo`/`_hi`, `wnat_mask_word_get_z`) are **retired** with the record (kept in the source, no longer evidence); `Dirty.lo_of`/`hi_of` were removed with the type. `u32_word_bit` stays **live**: `U32` is a native abstract type, so it remains a real `U32 ↔ Word(32)` bridge, not a `{==}`. The generic `wnat_*` value theory is unchanged.
...
| `window` (S1 step 3) | explicit resident set: `Window.resident_keys` (the window's 64 chunk keys, chunk-ordinal) and `Store.assemble_keys` (assemble over a key list instead of `assemble_go`'s loop bounds; additive, `assemble` unchanged); T95 (explicit set == implicit set) / T96 (reconstructs `Worldgen.build`). Also adapts `coreidea` rule 11 / `R11`: the terrain (`gen_terrain`) is shell-free, **but** the old "shell is a box/window property" reading is corrected by A.122 — only the *boundary* is window-relative; the *ground* is world-fixed (`G-scale-9`). Decision A for `G-scale-8` | A.99 |
...
| `window addressing` (S1 step 4c) | the assemble chain is window-aware: `assemble_chunk_w` addresses by `Window.win_index` (torus-free, no `&63`), threaded through `assemble_cells_w_sel`/`assemble_keys_go_w_sel`; the originals are wrappers over `Window.canonical()`, so `assemble` stays bit-identical (T18/T95/T96) while `Store.assemble_w`/`assemble_keys_w` assemble a *moved* window into its own array. T101: a one-chunk x-shifted window (global x 16..79) matches `Worldgen.gen` on all 262144 window-local cells (the `&63` fold would alias the far edge onto the near cells) | A.103 |
| `wrap range` (S1 step 5 / G-scale-2) | `src/nat.bend` develops the `Nat.mod` range bound from `Nat.divmod.go`'s `m + r` loop invariant (`n_lt_add_succ`/`go_rem_le`/`n_mod_lt`); `src/dirtyn.bend` adds `dwin_up_lt`/`dwin_down_lt`, which discharge the `mark_wake` reach hypotheses so `dwin_mark_wake_auto`'s wrap neighbours are in range without a caller side condition. Laws `nat_mod_lt`/`dwin_up_lt`/`dwin_down_lt`; T105 (n=6 range + mod-64 topology). Closes `G-scale-2` item (2); the live 64-row rewire and the `Mask ↔ Word(64)` isomorphism remain | A.104 |
| `g7` / `win_canonical_index` (G-scale-7) | `src/g7.bend` promotes the canonical-window relabelling from T81 to a law: for in-box `x,y,z`, `Window.win_index(Window.canonical(), x, y, z) == Grid.index(x, y, z)`. `g7_sub_zero` drops the origin subtraction (`U32.sub x 0 == x`), `g7_mask6` (via `G5.g5_and_mask_id` at width `2^6`) drops `Grid.index`'s three `&63` folds; both sides reduce to the same nested-`or` term. Law `win_canonical_index`; T102 (corners/edges) / T103 (the two bit facts) / T104 (the in-box hypothesis is necessary — at `x=64` the maps disagree, since `win_index` never folds). Closes `G-scale-7` | A.105 |
| `winshell` (S1 / G-scale-8) — **RETIRED (A.123)** | `src/winshell.bend` made the *box* contribute the shell at the window's own edge (the **wrong direction**: a window-local ground follows the camera; the module and its two laws are retired, no longer evidence): `border_of(lx,ly,lz)` (the shell predicate on window-local coords), `win_border w x y z` (its window instance), and `gen_at w x y z s` (bedrock on the window border, shell-free `gen_terrain` inside). `win_border_canonical` pins the canonical window to the absolute border (via `G7.g7_sub_zero`), `win_gen_canonical` pins `gen_at canonical` to `Worldgen.gen` bit-for-bit. T106 (canonical reproduces `gen` at corners/edges/interior) / T107 (a moved window's shell is window-local). Closes `G-scale-8`'s window-edge-shell piece; the live load/evict wiring remains | A.106 |
| `maskword` (S1 / G-scale-2 item 3) | `src/maskword.bend` proves the U32 ↔ `Word` bit-test core of the `Dirty.Mask ↔ Word(64)` isomorphism: `cat` builds the 64-bit word from `lo`/`hi`, and `u32_bit` proves `U32.is_ne(and(a, 1<<k), 0) == dwin_get(32, aWord, k)` from `and_wbit`, `cmp_wbit_zero`, `shlput_zero`/`shlput_true_bit0`/`shlput_bit`, `shl_bit_32`/`shln_one`, and `is_ne_wbit`/`is_ne_pick_wbit` (all over `Word`/`Bool`/`Cmp`). Also `cat_get_low`/`cat_get_high` (the 64-bit lo/hi split). Law `u32_word_bit`; T108 (32 sampled shifts on varied U32s) / T109 (`mask_word` places `lo` in bits 0..31, `hi` in 32..63). Composition residual: the U32/Nat index bridge (see §5.3) | A.107 |
| `window store` (S1 / G-scale-8) | `Store.window_store w s` is the window **load/evict policy** on the store: it rebuilds `s` onto exactly `w`'s resident keys — keep the overlap, load a missing chunk from the selected generator (`chunk_list_sel`), drop everything not resident in `w` (`load_keys_go_sel`/`cells_or_gen_sel`). It is the store `Store.assemble_w` assembles from. T120: a one-chunk x-move of the canonical window, then `assemble_w` over the windowed store, reconstructs `gen` on all 262144 cells; T121: the leaving chunk `(0,0,0)` is absent and the entering `(4,0,0)` present. The *law* (`assemble(evict s) == assemble s`) is the `G-scale-4` residual | A.108 |
| `wordnat` (S1 / G-scale-2 item 3) | `src/wordnat.bend` closes the `Dirty.Mask ↔ Word(64)` composition: the U32↔Nat index value theory over `Word.to_nat` (`wnat_w_cmp`: `Word.cmp n a b == Nat.cmp (to_nat a) (to_nat b)`; `wnat_two_pow`/`wnat_mask_bound`; `wnat_mask_step`/`_decomp`; `wnat_u32_lt32`/`_lt64`) plus the lo/hi branches, giving the law `mask_word_get`: for `y < 64`, `Dirty.is_dirty_y(m,y) == dwin_get(64n, Maskword.mask_word(m), U32.to_nat y)`. Reuses `maskword`'s `u32_bit`/`cat_get_low`/`cat_get_high` unchanged. T110–T117. The `Word.inc` value lemma / explicit `to_nat`/`from_nat` roundtrips are test-witnessed (T110/T116), not laws — the composition did not need them | A.109 |
...
| `wgrid`/`wmarg` (S1 slice W2) | the window margin / residency (`G-scale-1`). `src/wgrid.bend` gains `wgrid_axis_ok` (the per-axis residency test) and `wgrid_step_inside` is rewritten in `Window` terms (definitionally `Grid.step_inside`, so every existing guard proof stands); the model predicates `wgrid_resident`/`wgrid_target_resident` make residency explicit on a global coordinate. New `src/wmarg.bend` proves the bit brick `wmarg_mask6_high_zero` (`y < 2^6 ⇒ y & ~(2^6-1) = 0`, via `G5.g5_wand_mask_lt`), the per-axis `wgrid_axis_guard` (in-frame ⇒ guard `True`), the three-axis `wgrid_step_margin`, and `wgrid_canonical_resident` (canonical residency == in-box, via `g7_sub_zero`). Laws `wgrid_step_margin`/`wgrid_canonical_resident`; T126 (exhaustive over the guard's finite domain: local `0..63` × offset `{0,+1,-1}`, in-frame ⇒ guard and guard == in-frame) / T127 (moved-window residency both faces, face outward inert / inward live). W2 does not change runtime semantics (canonical is bit-identical); the driver chunk margin (`active ⇒ in-frame target`) is `W5` | A.111 |
| `w5` (S1 slice W5) | the live motion driver (the renderer's engine gate). `src/sim.bend` gains the pure `w5_` driver over the W1/W2 window: `w5_move` (`Window.shift` + `Store.window_store`, keep-overlap/load-entering/drop-leaving), `w5_checkpoint` (the array→store inverse of `Store.assemble_w`, gathering resident chunks in `Chunk.local` order via `Window.win_index`), `w5_step` (move → assemble → `Sim.ticks_w` → checkpoint, so a later move keeps ticked overlap state), and `w5_trace_step` (same plus an accumulated `List<Sim.Delta>` with global coordinates). No `Rules`/`Support`/`Ops`/`WGrid` change, no new module, no new law. Fast T130–T134 (structural, tick-free); native T135–T139 (moved-window tick vs canonical, global deltas, state across a move). **Residual:** the `runners/serve.bend` sidecar is not yet wired to the driver, and the driver does not assert the `Decision B` chunk margin | A.112 |
| `gsc1` (G-scale-1 residual) | the converse bridge. `src/bits.bend` proves the pointwise decomposition `y == (y & m) or (y & ~m)` (`gsc1_b_or_and_not`/`gsc1_word_or_and_not`), and `src/wmarg.bend` uses it (with `Word.w_cmp_eq` and `Bits.w_or_zero`) to prove `gsc1_axis_no_fold`: a passing `wgrid_axis_ok` guard forces `U32.and(U32.add(local,d), 63) == U32.add(local,d)` — the coordinate did not fold. Law `gsc1_axis_no_fold`. With `wgrid_axis_guard` (in-frame ⇒ guard) this makes the guard a proven residency test in both directions. Parallel track `gsc1` | A.113 |
| `evict` (G-scale-4 correctness) | the **lossless window move**. `Store.move_store w1 w2 s` keeps the overlap, loads `w2`'s residents from `gen`, and drops only a *leaving* chunk (`Window.in_resident w1` and not `w2`) that is sleeping **and** gen-equal (`sleeping_cells`/`gen_eq_cells`) — so a modified (non-gen-equal) chunk is never lost when it leaves the window. `Sim.w5_move` now uses it, and the step checkpoint folds onto the moved store (`w5_ck_store_on`) so the seam does not re-drop survivors; `window_store` is left intact. Fast T140–T144 (modified leaving chunk survives, gen-equal leaving dropped, entering loaded, overlap agrees, out-and-back step preserves the pile). **Residual:** the `G-scale-4` roundtrip law (`assemble(evict s) == assemble s`) is not yet proven — the store-level refinement lemmas do not exist | A.114 |
| `serve` (Bendview M5) | the **fixed-window live engine link**. `runners/serve.bend` replaces the one-tick demo with a continuous loop on the canonical window: HELLO → dense `SNAPSHOT` of `Fixtures.spawn(Sim.build())` → a per-tick `DELTA` loop (`Sim.tick_trace`, global coordinates) gated by the `bridge3` credit flow → sparse checkpoint `SNAPSHOT` + `PING` + `BYE` on STOP/EOF. `Bridge3Flow` gains a sticky `stop` flag; the reader treats a read failure as EOF and closes the control channel. IO-only, `src/` untouched. Native T145–T149 (six-tick served-delta replay reconstructs the ticked world, frame shape, global idx decode, kind ordering, STOP/credit accounting). **Residual:** the engine/writer split (`G-bridge-1`) and the reader's one-frame-per-read decode remain | A.115 |
| `serve` (frame length) | the sidecar could not emit its first `SNAPSHOT`: `bridge2_frame` measured the payload with non-tail `List.length` (`1n+List.length(...)`), so the dense body (1048864 byte-words) overflowed the machine stack right after `HELLO`. Fixed with a constant-stack tail-recursive `bridge2_len` accumulator, used by `bridge2_frame` and `bridge2_delta_frame`; fast T150 regression (frames a 262144-deep payload). A full `bend runners/serve.bend` run emits 1758768 bytes of exactly-framed HELLO/SNAPSHOT/DELTA×8/SNAPSHOT/PING/BYE. IO only, `src/` untouched | A.116 |
| `focus` (Bendview D1) | the **moving-window focus protocol**: inbound `FOCUS` (kind 8, base chunk origin), outbound `ORIGIN` (kind 9, window base chunk); the live loop carries `(Window, Store, deltas)` and, on a pending focus, drives `Sim.w5_trace_step`, emits a moved-window sparse `SNAPSHOT` (superseding prior deltas) and window-local `DELTA`s. `HELLO`/`SNAPSHOT` byte shapes unchanged; `runners/SERVE.md` §2/§2.1/§4/§8 updated. IO only, `src/` untouched, no new laws. Fast T151–T155. **Residual:** the native end-to-end moved-window replay (supervisor) and focus coalescing | A.117 |
| `g4` (G-scale-4 bricks) | the proof-relevant `U32.is_eq` reflection (`g4_gen_eq_cells_reflect`, with the exact length side conditions) and the per-chunk `Array` regen identity (`g4_assemble_chunk_regen`/`_chunk`, `g4_assemble_cells_present`): a stored chunk that passes `gen_eq_cells` assembles exactly as its pure-gen replacement. four laws (`g4_`), new `src/g4.bend`; fast T161–T165. **Residual:** the store-level `get`/`set` trie refinement and the full 64-key roundtrip remain open (see `G-scale-4`) | A.117 |
| `w3` (attempted, reverted) | the live `Dirty` rewire to `Dirtyn`'s `Word(64)` (SCALE §8 W3) was implemented on branch `w3`, passed the gate and the fast suite, but **hangs the native sim suite** at T4b; reverted from `master` (branch kept). `Dirty` stays the two-`U32` mask. Recorded in `G-scale-2` | A.117 |
| `focusnative` (Bendview D1 witness) | the native end-to-end witness for the focus path: T176 `ORIGIN` (kind 9) encodes the moved base chunk; T177 the moved sparse `SNAPSHOT` decodes to exactly the moved window's resident set (`32 + count*16388`); T178 the moved `DELTA` records replay the moved tick and satisfy `win_gx/gy/gz(w, idx) == (x,y,z)` (the T145/T147 relation on the moved window); T179 the canonical path is byte-unchanged; T180 the moved stream order `ORIGIN→SNAPSHOT→DELTA/PING/BYE`. Tests only; native T176–T180 (sim 40/40) | A.118 |
| `w3fix` (S1 step 5 item 1) | the live `Dirty` rewire to `Dirtyn`'s `Word(64)` **re-landed with the native cliff fixed**: `active_mask` accumulates the canonical two-`U32` limbs during the prime scan and materialises the `Word(64)` once, instead of a 64-step `Word.or` per scanned cell (was ~838M nodes over `settle(50)`; native T4b never finished). `Dirty` still holds the word, so `mask_word_get` stays definitional and canonical behaviour is bit-identical. `G-scale-2` item 1 closed. Fast T156/T157/T166; native 40/40 | A.118 |
| `g4store` (G-scale-4) | the store-level `get`/`set` trie refinement **closed**: `g4_wf` (leaf-depth well-formedness) preserved by `set_w`, `g4_get_set_same`/`_other` (same-key read-back; other-key invisibility with no WF needed), and full-depth wrappers; plus the per-key fold steps `g4_assemble_cells_same`/`_other`, `g4_keep_cells_drop`/`keep`. nine laws in `src/g4.bend`; fast T171–T175. **Residual:** the 64-key `evict`/`assemble` fold (see `G-scale-4`) | A.118 |
| `serve` (streaming control reader) | the sidecar control reader now **streams frames**: `File.read_bytes` is a read-*some*, so `bridge3_reader` accumulates bytes and extracts complete `[len][kind][payload]` frames one at a time (`bridge3_frame_need`/`_ready`/`_take`/`_rest`, with the 64 MiB cap), instead of decoding the whole read as one frame and dropping the rest. This unblocks Bendview D1 live flight: a `FOCUS` sent with the opening `CREDIT` is now applied (moved `ORIGIN` + `SNAPSHOT` emitted; verified on both the interpreter and the native binary). IO only, `src/` untouched, no new laws; fast T181. **Residual:** control is still folded only when the writer next needs credit (room-1 `Chan`), so a focus lands within a credit window, not instantly; the engine/writer thread split remains (`G-bridge-1`) | A.119 |
| `shellfree` (D5 / G-scale-8 live) | the live link now serves the **shell-free window** (decision A): `runners/serve.bend` builds the link world from `Support.pass_all(Worldgen.build_terrain())` and threads `Worldgen.terrain()` through the whole live/focus path (`bridge3_link_world`, `focus_initial`/`focus_next_state`, the FOCUS move, `focus_snapshot_sparse`, the checkpoint, both serve entry points). New `Sim.w5_{move,step,trace_step,ck_store}_sel` thread the selector; the existing box wrappers stay bit-identical for the reference mode. Witnesses: no bedrock at the canonical boundary planes; a moved window's entering chunk is `gen_terrain`, not the absolute `gen`; the report's exact `x=63`-interior case. Fast T182–T184, native T185–T189. Pure wiring, no new laws. **Caveat (`G-scale-9`):** it also removed the shell's *ground*, so the live build is born fully awake (~80k active cells) | A.120 |
| `stackfix` (D5 follow-up) | the shell-free live path is now **constant-stack under the interpreter**. The shell-free world has ~80879 active cells (vs 441 shelled), so the first live tick's ~80942 `WSet` deltas overflowed four right-recursive folds on the live `tick_trace_w` path (`Sim.filter_color`, `Sim.commit_trace`, `Sim.phases_trace`, `Rules.wake_writes`) plus `Serve.focus_delta_records`; all are now tail-recursive accumulators with one `List.reverse` to preserve order. `bend runners/serve.bend` (interpreted) exits 0 with a 2360540-byte stream **byte-identical** to the compiled runner. Fast T190–T193. Pure reordering, no new laws. **Latent (not live):** `Rules.commit_sets` (the non-trace `phases`/`phase` path) is still a right fold — only reached by the retired full-scan `phase` entry. **Caveat (`G-scale-9`):** the 80,879 figure is a *build* artifact (the missing world ground), not a sustained tick cost — measured: 80,438 active at build, 0 after one tick | A.121 |
...
- [x] **B1** Transport implementation · landed (A.94) · deps: `bridge` P0 · serves: —
  ...
  forked reader); the engine/writer thread split remains — `G-bridge-1` (the
  reader now streams frames, A.119).
...
- [ ] **S1** Chunk-window migration · **in progress** · deps: `scale` P3 · serves: scale
  ...
  **Step 3 landed (A.99):** an explicit resident set — `Window.resident_keys`
  ...
  the entering/leaving slabs (T99/T100). **Step 4c landed (A.103):** window-local
  addressing — `assemble_chunk_w` writes through `Window.win_index` (no `&63`),
  threaded through the `assemble_*` chain so `Store.assemble_w`/`assemble_keys_w`
  assemble a *moved* window into its own array (canonical `assemble` stays
  bit-identical, T18/T95/T96; T101 checks the moved window against `gen`).
  **Step 5 started (A.104):** the `Nat.mod` wrap-range bound is proven
  (`nat_mod_lt`) and its `dwin_up`/`dwin_down` instances discharge the
  `mark_wake` reach hypotheses (`dwin_up_lt`/`dwin_down_lt`; T105), closing
  `G-scale-2` item (2). **`G-scale-7` closed (A.105):** the canonical-window
  relabelling `win_index == Grid.index` is a law for in-box coordinates
  (`src/g7.bend`; T102–T104). **`G-scale-8` window-edge shell — RETIRED (A.123):**
  `src/winshell.bend` put the shell at the window's own local `0`/`63` edge
  (`win_gen_canonical`; old T106/T107, now withdrawn). The **wrong direction** — a
  window-local ground follows the camera — so the module and its laws are retired
  and no longer evidence.
  **`G-scale-2` item (3) core landed (A.107):** `src/maskword.bend` proves the
  U32 ↔ `Word` bit-test bridge (`u32_word_bit`) and the `cat` split.
  **`G-scale-8` store load/evict policy landed (A.108):** `Store.window_store`
  rebuilds the store onto a window's resident set (keep overlap, load entering,
  evict leaving; T120/T121). **`G-scale-2` item (3) closed (A.109):**
  ...
  through `Ops`/`Rules`/`Support`/`Sim` are in place. **W2 landed (A.111):** the
  guard is now explicitly the window-local residency test (`wgrid_axis_ok`) and
  `src/wmarg.bend` proves the in-frame ⇒ guard direction (`wgrid_step_margin`)
  and the canonical residency bridge (`wgrid_canonical_resident`); T126/T127.
  W1/W2 are **canonical-only** — the live path passes `Window.canonical()` and
  the guard is bit-identical to `Grid.step_inside`, so behaviour is unchanged.
  **W5 landed (A.112, parallel track `w5`):** the pure live motion driver
  (`w5_move`/`w5_step`/`w5_trace_step`, global deltas) — the renderer's engine
  gate; T130–T139. **`G-scale-1` converse closed (A.113, parallel track
  `gsc1`):** law `gsc1_axis_no_fold` (guard `True` ⇒ no fold). **`evict`
  landed (A.114):** the move is now lossless (`Store.move_store`; `w5_move`
  uses it), closing the boundary state-loss hole. **`serve` landed (A.115):**
  the fixed-window live link (continuous `DELTA` loop) — the renderer can go
  live on the fixed window now. **Next slices:** the moving-window focus
  protocol (renderer-supplied focus → `w5_trace_step`), then W3 (live dirty-set
  rewire, `G-scale-2` item 1) and W6 (torus retirement) — the full W1–W6 handoff
  is `SCALE.md` §8.** Each step gate-green; gap candidates
  `G-scale-1`..`G-scale-9` (`G-scale-3`, `G-scale-7` closed). Decision A
  (`G-scale-8`): the window / infinite mode samples shell-free
  `Worldgen.gen_terrain`, the closed box keeps its shell as a **reference mode**
  (`R11` adapted, A.99). **Correction (`G-scale-9`):** decision A kept the good
  half (terrain is a pure world-coordinate sampler) and dropped the bad half — it
  removed the shell's *ground* along with its *walls*. The ground is a **world**
  property, now tracked as `G-scale-9`; the window is not the ground.

  **Re-scoped remaining work (A.123).** `S1` now ends at the **world-fixed ground**
  (`G-scale-9`), not at a window-local shell. Concretely: (1) choose the ground's
  form (world-fixed bedrock plane / fixed-`y` boundary rule / void); (2) add a
  pure **world-coordinate** grounded sampler and make the live world use it, so the
  served window is born settled and its material matches the renderer's backdrop;
  (3) ground `Support.sup` against it — the boundary must mean one thing, "can't
  move *and* holds you up"; (4) regenerate the `.bgt` oracle with the ground and
  hand Bendview the new vectors; (5) then `W6` (torus retirement). `G-scale-8`'s
  window-edge-shell direction is retired; the load/evict machinery
  (`Store.window_store`, `move_store`) is unaffected.
...
| `G-scale-1` | unproven | `Grid.step_inside` conflates "off the window" with "off the world" | open | S1 | a moving window needs a margin/load rule so a rule target just outside the resident set is not silently inert; closes by a window-margin invariant plus a test/law that every `Rules.plan` target of a resident cell is resident or loaded (`SCALE.md` §6). **Decision B (SCALE.md §8):** keep the window ≥1 chunk larger than the active region, so every neighbour of a resident active cell is resident. **A.111 (W2) closed the guard/residency half:** `wgrid_step_inside` is now explicitly the window-local residency test (`wgrid_axis_ok`), and `src/wmarg.bend` proves the in-frame ⇒ guard direction (`wgrid_step_margin`, via `wmarg_mask6_high_zero`) plus the canonical residency bridge (`wgrid_canonical_resident`); T126 is exhaustive over the guard's finite domain and T127 witnesses a moved window's faces. **Still open:** the driver chunk margin (`active ⇒ in-frame target`, the driver moves whole chunks but asserts no margin) and wiring the driver into the live sidecar (`runners/serve.bend`). **A.112 (W5) landed the pure motion driver** (`w5_move`/`w5_step`/`w5_trace_step`, global deltas; T130–T139). **A.113 (`gsc1`) closed the converse bridge:** law `gsc1_axis_no_fold` (a passing `wgrid_axis_ok` forces `and(add(local,d),63) == add(local,d)`), so with `wgrid_axis_guard` the guard is a proven residency test in both directions. **A.114 (`evict`) made the move lossless** (no modified chunk is dropped at the leaving boundary), and **A.115 (`serve`) wired the fixed-window live link**. **Still open:** the moving-window focus protocol (the fixed window does not track the camera) |
| `G-scale-2` | unproven | `Dirty.Mask` is exactly 64 `y` rows | open (groundwork) | S1 | a window needs a window-sized mask. **A.100 groundwork (`dirtywin`):** new `src/dirtyn.bend` generalises the mask to `2^n` rows (`dwin_width`), with the `mark_wake` `y±1` reach proven (`dwin_mark_wake_self`/`_up`/`_down`) and the `n=6` instance agreeing with `Dirty` (T94); 9 `dwin_` laws. **Still open:** (1) the live 64-row path is not rewired to `Dirtyn`; (2) the general wrap-range lemma `Nat.is_lt(dwin_up/down(w,y), w)` needs `Nat.mod` range lemmas (only test-witnessed); (3) the `n=6` agreement is a test (T94), not a law — needs a `Dirty.Mask`(`lo`/`hi`) ↔ `Word(64)` isomorphism. **A.104 closed (2):** `src/nat.bend` proves the `Nat.mod` range bound from `divmod.go`'s `m + r` invariant (`n_lt_add_succ`/`go_rem_le`/`n_mod_lt`), and `Dirtyn.dwin_up_lt`/`dwin_down_lt` discharge the `mark_wake` reach hypotheses; laws `nat_mod_lt`/`dwin_up_lt`/`dwin_down_lt`, T105. **A.106/A.107/A.109 / `maskword` + `wordnat`:** the `Mask ↔ Word(64)` isomorphism is **closed**. `cat`/`mask_word` build the 64-bit word, `cat_get_low`/`cat_get_high` are the lo/hi split, `u32_word_bit` is the U32↔`Word` bit-test core (A.107), and `src/wordnat.bend` proves the U32↔Nat index value theory and the headline `mask_word_get` (`dwin_get(64, mask_word m, to_nat y) == is_dirty_y(m, y)` for `y < 64`; law, T110–T117). **Closed (3).** Item (1) the live 64-row rewire remains. **A.117 (W3 attempt, reverted):** the rewire to `Dirtyn`'s `Word(64)` (branch `w3`) passed the gate and the fast suite but **hangs the native sim suite** at T4b (`settled_hash(True)` = `settle(50)` then one more `Sim.tick`; `settled_hash(False)` completes, and `w3` alone reproduces it). The observation is a settled-world `Sim.tick` that does not terminate — not a wake-volume issue (a settled world has no wakes), so the cause is not yet pinned; `active_mask` and `dirty_todo` alone complete. Reverted from `master`; the branch is kept. Closes by an efficient, terminating `Word`-backed `Dirty` (or a `U32` fast path for `mark_wake`/`or_y`/`is_dirty_y`) with a native T4b check. **A.118 (W3 landed, `w3fix`):** the rewire is in `master`; the native T4b "hang" was a **performance cliff**, not a logic bug — `active_mask`'s per-cell `mark_if` forced a 64-step `Word.or` for all 262144 scanned cells per tick (~838M nodes over `settle(50)`), so the suite never finished. `active_mask` now accumulates the canonical two-`U32` limbs natively and materialises the `Word(64)` once; canonical behaviour is bit-identical, native sim 40/40 in ~13 s. **Item 1 closed** |
...
| `G-scale-4` | unproven | the resident set is implicit in `assemble_go`'s loop bounds | open (partial) | S1 | a window needs an explicit set plus a load/evict policy. **A.96 groundwork (`gscale2`):** `src/winset.bend` is a pure finite-set model (`winset_mem`/`insert`/`remove`/`all`, `winset_evict`, `winset_gen_cell`) with laws `winset_mem_insert_self`/`_other`/`winset_remove_insert`; T73–T75. Still open: the `assemble`/`evict` roundtrip law (`assemble(evict(s)) == assemble(s)` for a regenerable chunk) needs an `Array` update-identity under the `G5` boundary, and general membership-after-remove needs a proof-relevant `is_eq` eliminator (the `Bool.pick` non-dependence blocker). **A.114 (`evict`) closed the boundary state-loss hole:** `Store.move_store w1 w2 s` drops only a leaving chunk that is sleeping **and** gen-equal, so a modified chunk is never lost on a move; `Sim.w5_move` uses it, and T140–T144 witness the survival/load/overlap. **Still open:** the roundtrip *law* itself — `assemble(evict s) == assemble s` needs an `Array` update-identity under the `G5` boundary plus store-level `get`/`set` refinement lemmas, none of which exist. **A.117 (`g4`) landed the two bricks:** the proof-relevant `U32.is_eq` reflection (`g4_gen_eq_cells_reflect`, carrying the exact `fuel == List.length` side conditions that rule out the over-budget trivially-`True` case) and the per-chunk `Array` regen identity (`g4_assemble_chunk_regen`/`_chunk`, `g4_assemble_cells_present`; the update-identity turned out to be congruence — both runs perform the same write sequence — so no `Array.set` commutativity is needed). four laws in `src/g4.bend`, fast T161–T165. **Still open:** the store-level `get`/`set` trie refinement (`get(set(s,k,v),k) == Some{v}` is *false* for arbitrary stores — it needs a trie well-formedness invariant over the 32-bit key path) and the 64-key `evict`/`assemble` fold. **A.118 (`g4store`) closed the store-level `get`/`set` trie refinement:** `g4_wf` (every `SLeaf` reachable at a partial depth is absent) is preserved by `set_w` (`g4_set_preserves_wf`), and on a well-formed store `g4_get_set_same` gives `get(set(s,k,v),k) == Some{v}` while `g4_get_set_other` gives other-key invisibility (no WF needed); the full-depth wrappers and the per-key fold steps `g4_assemble_cells_same`/`_other`, `g4_keep_cells_drop`/`keep` land too. nine laws, T171–T175. **Still open:** the full 64-key fold — blocked by Bend's quantity discipline on a reusable Type-valued hypothesis function through the `assemble_go_sel` recursion, and by the checker's >45 s normalisation of `chunk_list_sel`'s 4096-leaf array |
| `G-scale-5` | performance | **reframed — this row was mis-stated.** The tick cost already tracks disturbance (`C3`); the cost modes are (a) the one-time build transient from the missing ground (`G-scale-9`: the shell-free live build flags ~80k cells active and crushes the boundary, A.121) and (b) seam/frame cost (the first `DELTA`, the per-`FOCUS` sparse `SNAPSHOT`), **not** a sustained phase fold — measured: the shell-free world has 80,438 active cells at build and **0** after one tick | open | S1 | gates live usability, not just the scale target. Closes by a grounded build (no transient; `G-scale-9`) plus a measurement of render-horizon vs simulation-window frame cost. "closes by measurement on a real window" assumed a sustained active-set cost that does not exist |
...
| `G-scale-7` | unproven | the canonical window map is a relabelling of `Grid.index` (`win_index(canonical,x,y,z) == Grid.index(x,y,z)`) is test-witnessed (T81), not proven | closed | S1 step 3 | a `U32.sub(x,0) == x` / mask-drop development over `Bits`/`Word` (reusing `G5.g5_and_mask_id` and the `gscale` field lemmas); closes by a law `win_canonical_index`. **Closed (A.105, `g7`):** `src/g7.bend` proves it for in-box `x,y,z` (`g7_sub_zero` drops the canonical origin subtraction, `g7_mask6`/`G5.g5_and_mask_id` drops the `&63` folds); law `win_canonical_index`, T102–T104. Note the registry's literal phrasing is false *outside* the box (`win_index` never folds), so the law carries the in-box hypotheses (T104) |
| `G-scale-8` | unproven | `Worldgen.gen` paints bedrock on the **absolute** planes `x/y/z ∈ {0,63}`, so in a moving window the stray wall is carried inside the resident set. **Correction (see `G-scale-9`):** the original reading — "there is no shell at the window's own edge, so make the *box* contribute the shell (window-local `0`/`63`)" — was the **wrong direction**: it makes the *ground* window-relative, i.e. gravity follows the camera. The legitimate half of the finding is that an absolute-world *boundary* cannot coexist with a moving window. The right split is terrain (pure world-coordinate sampler), boundary/closure (grid-or-window property), and ground (**world** property), three things the absolute shell did at once | open | S1, Bendview M3/M4 | separate the three things the absolute shell did at once: the **terrain** (a pure world-coordinate sampler — `Worldgen.gen_terrain`, already shell-free), the **boundary/closure** (a grid-or-window property: an off-window step is inert), and the **ground** (a **world**-fixed support base case, `G-scale-9`). The canonical window reproduces today's `gen` bit-for-bit, so the fixed-box laws/worlds stand; renderers get one shell-free sampler plus a world-fixed ground predicate. ~~Deciding also implies a `coreidea.md` rule-11 note (worldgen is window-relative, shell included) and a `PLAN.md` §4.1 update~~ — superseded: rule 11 now separates boundary from ground, and §4.1 `R11` is updated accordingly. Reported by Bendview (`../Bendview/docs/engine-report-gen-shell-and-window.md`). **Decision A (A.99):** two modes — the closed box stays the reference (`gen`, shell); the window / infinite mode samples shell-free `gen_terrain`. **A.100 landed the renderer oracle** (`genvec`: `bendverse-terrain.bgt`). **A.101/A.103 closed the plumbing:** the shell-free generator selector (step 4a) and the window-local addressing + motion primitives (steps 4b/4c) are landed. **A.106 landed the window-edge shell** (`src/winshell.bend`: `win_border`/`gen_at`, canonical reproduces `gen`; T106/T107). **A.120 (`shellfree`) wired the live link to the shell-free window:** `runners/serve.bend` builds the link world from `Support.pass_all(Worldgen.build_terrain())` and threads `Worldgen.terrain()` through the live/focus path (new `Sim.w5_*_sel` variants; the box wrappers stay for the reference mode), so `bendview --live` no longer draws the absolute bedrock walls and a moved window's entering chunks are shell-free. **Still open:** the **world-fixed ground** (`G-scale-9`) — the live store's entering-chunk sampler and the support seed must agree on a ground that does not move with the window. **Superseded direction:** the *window-relative* shell (`src/winshell.bend` `gen_at`) as the simulation boundary was a wrong turn (camera-following ground); `gen_at` remains useful only as a *renderer* outer-boundary hint, not as the sim's ground. Also open: the broader live-rules-off-`Grid` migration (`W6`) |
| `G-scale-9` | unproven (design) | **the world has no ground** — `Support`'s base case. The closed box's shell provided it (bedrock on the absolute planes); the shell-free live world has none, so `Support` reads every solid cell as unsupported: the live build flags **80,438** cells active and crushes the boundary (material hash 3→4), then one tick deactivates them (A.121). Closure (`C2`) says "off-grid = inert"; `Support` says "off-grid = not ground" — the two disagree about what the boundary means | open | S1 | the ground is a **world** property (the datum the support recursion bottoms out on), never the window edge (`gen_at`) — see `coreidea.md` rule 11. **Closes by** making the boundary mean one consistent thing ("can't move *and* holds you up") at a **world-fixed** place — a world-fixed bedrock plane, a fixed-`y` boundary rule, or a void, chosen as a decision — and grounding `Support.sup` against it. Consequence: generated terrain must be the sim's fixpoint, so the renderer's backdrop agrees with the served window. Reframes `G-scale-5` and `G-scale-8` |
| `G-bridge-1` | unproven | the sidecar's credit window and delta coalescing are specified (`SERVE.md` §5–§6) | open (partial) | B1 residual | **A.96 (`bridge3`) landed the accounting:** bounded queue, credit window (no overrun/wrap), drop-and-keyframe coalescing, idle PING, and a forked `Chan` reader so control is read continuously; T76–T80, plus a manual pipe run (4 credit frames → 4 credited frames). **Still open:** (1) the engine and writer are still co-resident in one IO thread — Base has no non-blocking `Chan.try_recv`/`try_send`, so a slow renderer can still stall the engine at the stdout write (the §5 failure mode); closes by a Base try-chan or the in-process C FFI; (2) **closed (A.119):** `bridge3_reader` now accumulates bytes and extracts complete `[len][kind][payload]` frames one at a time (`bridge3_frame_need`/`_ready`/`_take`/`_rest`; T181), so batched/split frames no longer mis-parse — the report's "count-blocking read" diagnosis was wrong (`File.read_bytes` is read-*some*), but the streaming fix is what was needed; (3) no `stall_ms`/`ping_ms` flags. Note: control is folded only when the writer next needs credit, so a `FOCUS` lands within a credit window rather than instantly |
...
| `G-bridge-4` | unproven (defect) | `runners/serve.bend`'s `bridge2_frame` measured the payload with non-tail `List.length` (`1n+List.length(...)`), so the dense first `SNAPSHOT` (1048864 byte-words) exhausted the machine stack right after `HELLO` and no real stream could be captured | closed | B1 / Bendview M5 | **closed (A.116):** `bridge2_len`/`bridge2_len_go` is a tail-recursive accumulator and `bridge2_frame`/`bridge2_delta_frame` use it; fast T150 frames a 262144-deep payload (the old `List.length` faults there), and a full `serve.bend` run emits the exact 1758768-byte stream. Reported by Bendview (`../Bendview/docs/engine-report-serve-frame-length.md`) |
