...
| `G-scale-8` | unproven | `Worldgen.gen` paints bedrock on the **absolute** planes `x/y/z ∈ {0,63}`, so in a moving window the stray wall is carried inside the resident set and there is no shell at the window's own edge — an absolute world edge cannot coexist with a moving window (or an infinite render horizon) | open | S1, Bendview M3/M4 | separate terrain from the box: `Worldgen.gen_terrain` is already shell-free, so make the *box* contribute the shell (window-local `0`/`63`), with the canonical window reproducing today's `gen` bit-for-bit so the fixed-box laws/worlds stand; expose one shell-free sampler + a border predicate the renderer can use for non-resident cells. Deciding also implies a `coreidea.md` rule-11 note (worldgen is window-relative, shell included) and a `PLAN.md` §4.1 update. Reported by Bendview (`../Bendview/docs/engine-report-gen-shell-and-window.md`). **Decision A (A.99):** two modes — the closed box stays the reference (`gen`, shell); the window / infinite mode samples shell-free `gen_terrain`. **A.100 landed the renderer oracle** (`genvec`: `bendverse-terrain.bgt`). Still open: wire the window's regeneration to `gen_terrain` (a generator selector through `Chunk`/`Store`) and implement window motion (step 4) |
