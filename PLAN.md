...
- `bend f.bend` checks the file, then runs `main`. `-o out` builds. `--checkup`
  ...
  its 114 laws as TODOs. Use `bend PROOF.bend`, never `--checkup`, for the gate.
...
114 laws, every one discharged. Six are reflexivity proofs (`{==}`, both sides
...
- [ ] **S1** Chunk-window migration · **in progress** · deps: `scale` P3 · serves: scale
  ...
  **Step 5 started (A.104):** the `Nat.mod` wrap-range bound is proven
  ...
  `G-scale-2` item (2). Remaining: **step 5** live 64-row rewire + the
