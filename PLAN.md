...
- `bend f.bend` checks the file, then runs `main`. `-o out` builds. `--checkup`
  ...
  its 120 laws as TODOs. Use `bend PROOF.bend`, never `--checkup`, for the gate.
...
120 laws, every one discharged. Nine are reflexivity proofs (`{==}`, both sides
...
- [ ] **S1** Chunk-window migration · **in progress** · deps: `scale` P3 · serves: scale
  ...
  **Step 5 started (A.104):** the `Nat.mod` wrap-range bound is proven
  ...
  Remaining: **step 5** live 64-row rewire + the `Mask ↔ Word(64)` isomorphism,
  **step 6** torus-law retirement, `G-scale-8`'s live load/evict wiring. Each step gate-green; gap candidates
