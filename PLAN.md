...
- `bend f.bend` checks the file, then runs `main`. `-o out` builds. `--checkup`
  ...
  its 75 laws as TODOs. Use `bend PROOF.bend`, never `--checkup`, for the gate.
...
75 laws, every one discharged. Six are reflexivity proofs (`{==}`, both sides
...
| `R2` | Conservation: swap/transform only | `array_point_write_preserves_count` `array_point_write_count_balance` `array_mov_swap_preserves_count` `array_support_write_preserves_count` `array_fall_write_preserves_count` `array_mov_write_preserves_count` `array_wake_write_preserves_count` `array_deactivate_write_preserves_count` `write_selector_preserves_count` | T2 T21 T38 T39 | count rules ops selector |
...
| `R2` | Conservation: swap/transform only | conforming (count) / unproven (write-*site* coverage) | count laws closed (`V4`). The write-site *enumeration* evidence is split: `G10` is closed but its subject `Rules.step` was retired by A.63, and the live `Rules.plan` enumeration is the **open** `G14`. `Selector.array_write_preserves_count`/`array_write_preserves_phi` (A.78) cover every material-preserving write primitive unconditionally, which is what `G14` needs from the write side; what remains for `G14` is that `plan` emits only those. |
...
| `V3d` | generic material-preserving write family: the four hand-written `PT` recursions (`Tick.chg_support`/`chg_fall`/`chg_activate`/`chg_deactivate`, each with its own `chg_X_if` and `array_X_write`) are replaced by one traversal over a `Data`-kinded selector (`Selector.WrSel`, `apply_w`), with `mat_pres` as the whole per-site obligation. Laws `write_selector_preserves_phi`/`_count` hold for every constructor at once; `array_fall_write_preserves_phi`, `array_mov_write_preserves_phi` and `array_deactivate_write_preserves_phi` are new statements — `set_fall0` and `deactivate` are emitted by the live `Rules.plan` every tick and had no array-level Φ law (only the retired `step_m` covered them). A selector is required rather than a function parameter because Bend closures are affine (callable at most once) while a write is applied at every leaf. Also lands the first executable Φ oracle: `Settle.array_phi` is proof-side and `suml` is not tail-recursive, so it overflowed on a 2^18-cell world and the flagship property had never been checked end to end. T39 pins the oracle to `Settle.pot_at`; T38 runs it — Φ is non-increasing across 60 real ticks and strictly decreases | A.78 |
...
| `G14` | unproven | V0-1's `Rules.plan` write-*site* enumeration is by inspection, not mirrored: the write primitives it emits (`mov`, `crush_if_rock`, `set_fall0`, `deactivate`, `wake`) each have proven Φ/conservation effects, but that `plan`'s write set is exactly those is not a theorem | open | G5 | mirror `plan` on `PT` (a fresh enumeration, since `step_m` mirrored the retired `Rules.step`), or prove the `plan`→write-set correspondence directly. **A.78 narrowed the write side:** `Selector.array_write_preserves_phi`/`_count` now cover *every* material-preserving primitive unconditionally in one law (including `set_fall0` and `deactivate`, which previously had no array-level Φ law at all — their only coverage was the retired `step_m`), so what `G14` still owes is only that `plan` emits no other kind of write. The `step_m` mirror is now marked RETIRED in `LAWS.bend` so it cannot be read as live coverage |
